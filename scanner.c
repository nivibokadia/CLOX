...
static void skipWhitespace() {
    for (;;) {
        ...
        case '\n':
            ...
            break;
        case '/':
            if (peekNext() == '/') {  // Use single quote for character comparison
                while (peek() != '\n' && !isAtEnd()) advance();
            } 
            else return;
            break;
...
static TokenType checkKeyword(int start, int length, const char* rest, TokenType type){
    if(scanner.current - length == start + length && memcmp(scanner.start + start , rest, length)){
        return type;
    } 
...
static Token string(){
    while (peek() != '"' && !isAtEnd()) {
        if (peek() == '\n') scanner.line++;  // Use parentheses to call the function
        advance();
    }
