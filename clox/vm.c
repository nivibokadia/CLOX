...
static void runtimeError(const char* format, ...){
    ...
    for (int i = vm.frameCount - 1; i >= 0; i--) {
        ...
        size_t instruction = frame->ip - function->chunk.code - 1;   // -1 because the IP is sitting on
        fprintf(stderr, "[line %d] in ",                             //the next instruction to be executed.
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_CALL: {
                ...
                frames = &vm.frames[vm.frameCount-1];
                break;
            }
            case OP_GREATER:{
                BINARY_OP(BOOL_VAL, >);
