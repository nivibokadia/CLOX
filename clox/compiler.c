...
typedef enum{
    PREC_NONE,              //none                         (no precedence)
    PREC_ASSIGNMENT,        //assignment operator =
    PREC_OR,                //logical or ||
    PREC_AND,               //logical and &&
    PREC_EQUALITY,          //equality == or !=
    PREC_COMPARISON,        //comparison <,>,<=,>=
    PREC_TERM,              //addition and subtraction + and -
    PREC_FACTOR,            //multiply and divide * and /
    PREC_UNARY,             //negate -
    PREC_CALL,              //function calls
    PREC_PRIMARY            //identifiers and literals     (highest precendence)
}Precedence;
...
typedef struct ClassCompiler {
    struct ClassCompiler* enclosing;    //pointer to ClassCompiler for enclosing class, if any
    Token name;
    bool hasSuperclass;
}ClassCompiler;
...
static void block(){                   //compiling a block including closing braces
...
static void function(FunctionType type){
    ...
    if(!check(TOKEN_RIGHT_PAREN)){
        ...
        uint8_t paramConstant = parseVariable("Expect a parameter name.");
        defineVariable(paramConstant);
        }while(match(TOKEN_COMMA));
    }
    consume(TOKEN_RIGHT_PAREN, "Expect a ')' after parameters.");
    consume(TOKEN_LEFT_BRACE, "Expect '{' before function body.");
    block();
    ObjFunction* function = endCompiler();  //call endCompiler after reaching end of func.
...
static void returnStatement(){
    ...
    if(match(TOKEN_SEMICOLON)){
        ...
        if(current->type == TYPE_INITIALIZER){
            error("Initializer can't return a value");
        }
        expression();
...
static void forStatement(){
    ...
    if(TOKEN_SEMICOLON){ }    //condition clause
    ...
    int exitJump = -1;
    if(!match(TOKEN_SEMICOLON)){        //initializer clause
        expression();
        consume(TOKEN_SEMICOLON, "Expect ';' after 'loop'.");
